...
class ProductionDataCollector:
    def __init__(self, max_matches=3, delay_between_requests=5, max_games_per_match=10):
        ...
        logger.info(f"Conservative settings: {max_matches} matches, {delay_between_requests}s delay")
    ...
    def collect_with_rate_limiting(self):
        ...
        try:
            ...
            for match_idx, match_url in enumerate(match_links):
                ...
                game_links = basic_scraper.get_game_links_from_match(match_url)[:self.max_games_per_match]
                ...
                for game_idx, game_url in enumerate(game_links):
                    ...
                    if game_data and game_data.get('js_data', {}).get('scores'):
                        ...
                        if valid_images:
                            ...
                            logger.info(f"    ✓ Collected {len(valid_images)} images with scores: {game_data.get('js_data', {}).get('scores', [])}")
                        else:
                    ...
                    if game_idx < len(game_links) - 1:  # Don't delay after last game
                        logger.info(f"    Waiting {self.delay_between_requests}s before next game...")
