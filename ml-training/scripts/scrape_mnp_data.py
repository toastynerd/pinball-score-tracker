...
class MNPScraper:
    ...
    def scrape_game_data(self, game_url):
        ...
        try:
            ...
            logger.info(f"Game {game_url}: {len(game_data['images'])} images, {len(game_data['scores'])} score fields")
            return game_data
...
if __name__ == "__main__":
    ...
    for i, game in enumerate(data[:3]):  # Show first 3 games
        print(f"Game {i+1}: {len(game.get('images', []))} images, {len(game.get('scores', []))} scores")