...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('persistent_scraper.log'),
...
class PersistentScraper:
    ...
    def load_state(self):
        ...
        if os.path.exists(self.state_file):
            with open(self.state_file, 'r') as f:
                state = json.load(f)
                logger.info(f"Resuming session from: {state.get('last_processed_match', 'beginning')}")
                return state
        else:
    ...
    def collect_all_data(self):
        ...
        try:
            ...
            remaining_matches = [
                match for match in all_match_links 
                if match not in self.state["completed_matches"] and 
                   match not in self.state["failed_matches"]
            ]
            ...
            for match_idx, match_url in enumerate(remaining_matches):
                ...
                try:
                    ...
                    if match_idx < len(remaining_matches) - 1:
                        match_delay = self.calculate_dynamic_delay(self.state["statistics"]["total_requests_made"]) * 2
    ...
    def process_match(self, basic_scraper, match_url, match_idx):
        ...
        try:
            ...
            for game_idx, game_url in enumerate(game_links):
                ...
                try:
                    ...
                    if game_idx < len(game_links) - 1:
                        game_delay = self.calculate_dynamic_delay(self.state["statistics"]["total_requests_made"])
            ...
            if match_data:
                ...
                logger.info(f"  ✓ Match complete: {games_with_data}/{len(game_links)} games had data")
            else:
    ...
    def generate_final_report(self):
        ...
        report = {
            ...
            "data_quality": {
                ...
                "success_rate": self.state["statistics"]["games_with_data"] / max(1, 
                    self.state["statistics"]["games_with_data"] + self.state["statistics"]["games_without_data"])
            },
            "rate_limiting": {
                "base_delay": self.base_delay,
                "max_delay": self.max_delay,
                "average_delay": self.state["statistics"]["total_requests_made"] * self.base_delay / max(1, total_time.total_seconds())
            }
        }
...
def main():
    ...
    parser.add_argument("--base-delay", type=int, default=60,
    ...
    parser.add_argument("--session-name", type=str,
                        help="Name for this scraping session")
