...
class ModelVersioningSystem:
    ...
    def register_model(
        ...
        if model_id in self.registry["models"]:
            ...
            if existing_hash == model_hash:
                logger.info(
                    f"Model {model_id} already exists with same hash. Skipping registration."
                )
                return model_id
            else:
                logger.warning(
                    f"Model {model_id} exists but with different hash. Creating new version."
                )
    ...
    def list_models(
        self, name_filter: Optional[str] = None, tag_filter: Optional[str] = None
    ) -> List[Dict]:
        """
...
def main():
    ...
    versioning = ModelVersioningSystem(base_dir=args.base_dir, s3_bucket=args.s3_bucket)
    ...
    if args.command == "register":
        ...
        model_path = versioning.download_model(
            model_id=args.model_id, output_dir=args.output_dir, force=args.force
        )
