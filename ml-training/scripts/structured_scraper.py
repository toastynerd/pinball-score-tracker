...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('structured_scraper.log'),
...
class StructuredMNPScraper:
    def __init__(self, base_delay=5, max_delay=30, session_name="structured_collection"):
    ...
    def get_week_matches(self, week_num):
        ...
        if not week_matches and week_num <= 14:
            ...
            end_idx = start_idx + matches_per_week if week_num < 14 else len(all_matches)
    ...
    def process_week(self, week_num):
        ...
        for match_idx, match_url in enumerate(week_matches, 1):
            ...
            match_images, match_games = self.process_match(match_url, week_num, match_idx)
        ...
        logger.info(f"✅ WEEK {week_num} COMPLETE: {week_images} images, {week_games} games")
    ...
    def process_match(self, match_url, week_num, match_idx):
        ...
        for round_num in range(1, 5):  # Rounds 1-4
            ...
            logger.info(f"🎯 WEEK {week_num} - MATCH {match_idx} - ROUND {round_num}: {len(round_games)} games")
            
            round_images = self.process_round(round_games, week_num, match_idx, round_num)
    ...
    def process_round(self, round_games, week_num, match_idx, round_num):
        ...
        for game_idx, game_url in enumerate(round_games, 1):
            logger.info(f"🎲 WEEK {week_num} - MATCH {match_idx} - ROUND {round_num} - GAME {game_idx}/{len(round_games)}")
            logger.info(f"🎲 Game URL: {game_url}")
            
            game_images = self.process_game(game_url, week_num, match_idx, round_num, game_idx)
            ...
            if game_images == 0:
                ...
                logger.warning(f"⚠️  Consecutive failures: {self.state['consecutive_failures']}/{self.state['max_consecutive_failures']}")
                
                if self.state["consecutive_failures"] >= self.state["max_consecutive_failures"]:
                    logger.error(f"🛑 CIRCUIT BREAKER TRIGGERED after {self.state['consecutive_failures']} consecutive failures")
                    logger.error(f"🛑 Failed games: {[game['url'] for game in self.state['failed_games'][-10:]]}")
                    
                    # Try recovery: recreate selenium driver and skip to next match
                    logger.info(f"🔄 Attempting recovery: recreating driver and skipping to next match...")
                    try:
                        if hasattr(self.selenium_scraper, 'driver') and self.selenium_scraper.driver:
                            self.selenium_scraper.driver.quit()
                        self.selenium_scraper.setup_driver()
                        self.state["consecutive_failures"] = 0  # Reset after recovery attempt
                        logger.info(f"✅ Recovery successful, continuing from next match")
                        return round_images  # Exit this round, continue to next
                    except Exception as recovery_error:
                        logger.error(f"❌ Recovery failed: {recovery_error}")
                        raise Exception(f"Circuit breaker triggered and recovery failed: {self.state['consecutive_failures']} consecutive failures")
            else:
    ...
    def get_failure_report(self):
        ...
        if self.state["last_successful_game"]:
            ...
            lines.append(f"Last successful game: {last_success['url']} at {last_success['timestamp']}")
        
        lines.append("Recent failed games:")
        for game in self.state["failed_games"][-5:]:
            lines.append(f"  - {game['url']} (W{game['week']} M{game['match']} R{game['round']} G{game['game']})")
    ...
    def run_structured_collection(self, start_week=1, end_week=14):
        ...
        logger.info(f"📊 Total: {total_images} images from {total_games} games across {end_week - start_week + 1} weeks")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Structured MNP scraper following site organization")
    parser.add_argument("--start-week", type=int, default=1, help="Week to start from (1-14)")
    parser.add_argument("--end-week", type=int, default=14, help="Week to end at (1-14)")
    parser.add_argument("--base-delay", type=int, default=5, help="Base delay between requests")
    parser.add_argument("--max-delay", type=int, default=30, help="Max delay between matches")
    parser.add_argument("--session-name", type=str, default="week1to14", help="Session name")
