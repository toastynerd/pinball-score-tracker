    ...
    def process_dataset(self):
        ...
        for game_idx, game in enumerate(games_data):
            ...
            for img_idx, img_path in enumerate(game.get('local_images', [])):
                ...
                src_path = os.path.join(self.input_dir, img_path) if not os.path.isabs(img_path) else img_path
                if not os.path.exists(src_path):
                    # Try alternative path
                    clean_path = img_path.replace(f'{self.input_dir}/', '').replace(self.input_dir, '')
